...
{
    uint8_t ret = 0;
    ret = I2C_WriteBits(MPU6050_ADDRESS, MPU6050_RA_PWR_MGMT_1, MPU6050_PWR1_CLKSEL_BIT, MPU6050_PWR1_CLKSEL_LENGTH, source);
    if (ret == 0)
    {
        printf("Cannot SetClockSource\r\n");
        delay_ms(500);
        NVIC_SystemReset();
    }
    else
    {
        printf("SetClockSource OK!\r\n");
    }
}
...
{
    I2C_WriteBit(MPU6050_ADDRESS, MPU6050_RA_PWR_MGMT_1, MPU6050_PWR1_SLEEP_BIT, enabled);
}
...
{
    I2C_ReadBytes(MPU6050_ADDRESS, MPU6050_RA_WHO_AM_I, 1, buffer);
    return buffer[0];
}
...
{
    I2C_WriteBit(MPU6050_ADDRESS, MPU6050_RA_USER_CTRL, MPU6050_USERCTRL_I2C_MST_EN_BIT, enabled);
}
...
{
    I2C_WriteBit(MPU6050_ADDRESS, MPU6050_RA_INT_PIN_CFG, MPU6050_INTCFG_I2C_BYPASS_EN_BIT, enabled);
}
...
{
    uint8_t temp[1] = {0};
    I2C_Read(MPU6050_ADDRESS, 0x75, 1, temp);
    printf("mpu_set_sensor complete ......\r\n");
    if (temp[0] != (MPU6050_ADDRESS >> 1))
        NVIC_SystemReset();
    if (!mpu_init())
    {
